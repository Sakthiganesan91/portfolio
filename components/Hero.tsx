import React from "react";
import ShimmerButton from "./ui/shimmer-button";
import { Spotlight } from "./ui/Spotlight";
import { TextGenerateEffect } from "./ui/text-generate-effect";

function Hero() {
  const words = "Sakthiganesan, Full Stack Developer";
  return (
    <div className="pb-20 pt-10">
      <div>
        <Spotlight
          className="-top-40 -left-10 md:-left-32 md:-top-20 h-screen"
          fill="white"
        />
        <Spotlight
          className="top-10 left-full h-[80vh] w-[50vw]"
          fill="purple"
        />
        <Spotlight
          className="top-28 left-80 md:-left-32 h-[80vh] w-[50vw]"
          fill="blue"
        />
      </div>
      <div className="h-screen w-full dark:bg-black-100 bg-white  dark:bg-grid-white/[0.002] bg-grid-black/[0.03] absolute flex items-center justify-center top-0 left-0">
        <div className="absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black-100 bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]" />
      </div>
      <div className="flex justify-center relative my-20 z-10">
        <div className="max-w-[89vw] md:max-w-2xl lg:max-w-[60vw] flex flex-col items-center justify-center">
          <h2 className="uppercase tracking-widest text-xs text-center text-blue-200 max-w-80">
            Blending Technology with Perfection
          </h2>
          <TextGenerateEffect
            words={words}
            className="text-center text-[40px] md:text-5xl lg:text-6xl"
          />
          <p className="text-center text-white md:tracking-wider mb-4 text-sm md:text-lg lg:text-2xl">
            Guy Passionate about Computer Technology
          </p>
          <div className="flex">
            <div className="mx-2">
              <a href="https://github.com/Sakthiganesan91">
                <ShimmerButton buttonName="Github" />
              </a>
            </div>
            <div>
              <a href="https://www.linkedin.com/in/sakthiganesang/">
                <ShimmerButton buttonName="Linkedin" />
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Hero;
